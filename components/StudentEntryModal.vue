<template>
    <div>
        <b-button v-b-modal.StudentEntryModal variant="primary">Add Student</b-button>

        <b-modal id="StudentEntryModal" title="Student Entry" ok-title="Save Student" @ok="onSubmit">
            <form action="#">
...
export default {
    ...
    methods: {
        async onSubmit() {
            ...
            .then((res)=>{
                if(res.status==202) {
                    alert('Successfully added an student')
                    this.$emit('onAdd')
                }
            })
        }
